...
coplot(car1$mpg ~ car1$wt | as.factor(car1$cyl), data = car1,
...
par(mfrow=c(2,2))
plot(hp, mpg,  col=as.integer(car$cyl), pch=19)
# best fit linear line, lwd = 두께 lty = 선 타입
abline(lm(mpg~hp), col="red", lwd=2, lty=1)
...
#par(mfrow=c(2,1))
plot(hp1$hp, hp1$mpg, col=as.integer(hp1$cyl), main = 'mpg w/ hp < 50', pch=19)
abline(lm(hp1$hp~hp1$mpg), col='red', lwd=2, lty=1)

plot(hp2$hp, hp2$mpg, col=as.integer(hp2$cyl), main = 'mpg w/ hp >= 50', pch=19)
abline(lm(hp2$hp~hp2$mpg), col='red', lwd=2, lty=1)



# lowess : smoothed line, nonparmetric fit line (locally weighted polynomial regression)
par(mfrow=c(1,1))
plot(disp, mpg,  col=as.integer(car$cyl), pch=19)
abline(lm(mpg~disp), col="blue", lwd=2, lty=1)
lines(lowess(disp, mpg), col="red", lwd=3, lty=2)
help(lowess)
